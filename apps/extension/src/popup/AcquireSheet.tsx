 ...
 *
 * Both CSPR and the demo USDC (CEP-18) are dispensed by Baret's own
 * treasury-backed faucet (one tap → wallet.airdrop / wallet.airdropToken RPC
 * → apps/server POST /demo/faucet[-token]), with the captcha-gated cspr.live
 * faucet kept as a CSPR fallback. Rendered as a full-surface overlay so it
 * works in the popup and (wrapped) in options.
 */
...
import { CSPR_LOGO } from "../shared/tokens";
    ...
    <div className="absolute inset-0 z-30 flex flex-col" style={{ background: "var(--bg)" }}>
      ...
      <div className="flex-1 px-4 py-4 flex flex-col gap-3 overflow-y-auto">
        ...
        <section className="card !p-4">
          <CardHeader symbol="CSPR" title="Casper (CSPR)" logo={CSPR_LOGO} />
...
function CardHeader({ symbol, title, logo }: { symbol: string; title: string; logo?: string }) {
  return (
...
function TokenAcquireCard({
  token, network, address, onFunded,
}: {
  token: TokenDef;
  network: string;
  address: string;
  ...
  const requestToken = async () => {
    ...
    setMsg(null);
    try {
      const r = await rpc.call("wallet.airdropToken", { packageHash: token.packageHash });
      setMsg(`Sent ${r.amount.toLocaleString()} ${token.symbol} — arriving in a few seconds.`);
      onFunded?.();
    } catch (e) {
      setErr(e instanceof Error ? e.message : String(e));
    } finally {
  ...
  return (
    <section className="card !p-4">
      <CardHeader symbol={token.symbol} title={token.name} logo={token.logo} />

      {isTestnet ? (
        <>
          <p className="text-xs text-text-muted leading-relaxed mb-3">
            Get free test {token.symbol} from Baret's treasury faucet — same
            cooldown as the CSPR faucet.
          </p>
          <button onClick={requestToken} disabled={busy} className="btn-primary w-full !py-2.5 text-sm">
            {busy ? <Loader2 size={13} className="animate-spin" /> : <Droplet size={13} />}
            {busy ? "Requesting…" : `Request 1,000 ${token.symbol}`}
          </button>
        ...
        <>
          <p className="text-xs text-text-muted leading-relaxed mb-3">
            CEP-18 stablecoin used for x402 micropayments. No faucet on mainnet —
            receive it by transfer from whoever holds it.
          </p>
