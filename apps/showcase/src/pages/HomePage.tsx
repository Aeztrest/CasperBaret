/**
 * Baret home — white-first landing with Casper red + ink-black.
...
function LiveAnalysisCard() {
  const findings = [
    { sev: "danger", icon: AlertTriangle, label: "Transfers CSPR to unknown wallet", detail: "10.0 CSPR → 3kBz…q9F" },
    { sev: "warn",   icon: ShieldAlert,   label: "Sets unlimited CEP-18 approval",   detail: "CEP-18 · spender unverified" },
    { sev: "ok",     icon: CheckCircle2,  label: "Contract reputation verified",     detail: "Verified Casper router" },
  ];
  ...
  return (
    ...
    >
      <div className="absolute -inset-6 rounded-[2rem] opacity-60 blur-2xl"
           style={{ background: "radial-gradient(closest-side, rgba(225,20,40,0.18), transparent 70%)" }} />
          ...
          <div className="space-y-2">
            {findings.map((f, i) => (
              <motion.div
                ...
                style={{
                  borderColor:
                    f.sev === "danger" ? "rgba(225,20,40,0.45)" :
                    f.sev === "warn"   ? "rgba(251,151,160,0.30)" :
                                         "rgba(255,255,255,0.10)",
                  background:
                    f.sev === "danger" ? "rgba(225,20,40,0.10)" :
                    f.sev === "warn"   ? "rgba(251,151,160,0.06)" :
...
function ProblemSolution() {
  const rows = [
    {
      moment: "Blind sign",
      old:    "A plain Casper wallet shows a raw contract package hash and a Confirm button.",
